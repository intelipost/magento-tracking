...
class Intelipost_Tracking_WebhookController extends Mage_Core_Controller_Front_Action
{
    public function indexAction()
    {
        ...
        $trackingPreDispatch = Mage::getStoreConfig("intelipost_tracking/webhook/tacking_pre_dispatch");
        $trackingPostDispatch = Mage::getStoreConfig("intelipost_tracking/webhook/tacking_post_dispatch");
        ...
        if ($webhookActive) {
            ...
            if ($this->getRequest()->getHeader('api-key') == Mage::helper('basic')->getDecriptedKey('apikey')) {
                ...
                if ((in_array($state, $pre_dispatch_events) && $trackingPreDispatch)
                    || in_array($state, $post_dispatch_events) && $trackingPostDispatch) {
                    Mage::log("Status allowed");
